...
$(document).ready(function () {/*Функцию, которая запустится после того, как DOM полностью загрузился.*/
    ...
    $("#iPhone").click(function () {
        loadFile("iPhoneOS.txt");
    });

    $("#Android").click(function () {
        loadFile("AndroidOS.txt");
    });
...
function getFileName(OSId) {
    ...
    switch (OSId.trim()) {
        case "iPhone OS":
            fileName = "iPhoneOS.json";
            break;
        case "Android OS":
            fileName = "AndroidOS.json";
            break;
        case "Bada OS":
            fileName = "BadaOS.json";
            break;
        case "Symbian OS":
            fileName = "SymbianOS.json";
            break;
    }
    return fileName;
}
